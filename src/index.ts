...
const randomizeGrid = (prob = 0.5) => (grid = <number[]>randomSeq(W * H, prob));

const randomizeRules = () => {
  rules = <number[]>randomSeq(18);
  randomizeGrid();
  setHash();
};
...
export const convolve = (
  ...
  transduce<number[], number, number[]>(
    comp(
      ...
      mapIndexed((i: number, x: number) => rules[x + src[i] * rstride])
    ),
    push(),
...
const ruleBoxes = (prefix: string, i: number, rstride = 9) => [
  ...
  ...rules
    ...
    .map((rule, j) =>
      checkbox(rule, e => setRule(i, j, (<HTMLInputElement>e.target).checked))
    )
];
